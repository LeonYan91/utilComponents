...
var fastDFSId = getQueryString("fastDFSId");

var basePath = '/' + window.location.pathname.split('/')[1];

//TODO　目前支持的文件为ppt,excel,word,txt,pdf，不支持的文件直接下载

var previewFileOrDownload = function(fastId){
    ...
    var jumpPath = null;
    var type = fastId.substring(fastId.lastIndexOf(".") + 1);
    if(type == 'doc' || type == 'docx' || type == 'xls' || type == 'xlsx' || type == 'ppt' || type == 'pptx' || type == 'txt' || type == 'pdf'){
        //符合条件的文件格式，跳转到预览
        jumpPath = "/static/yineng/components/22-ynuiPdfViewer/web/viewer.html?fastDFSId="+fastId;
        isSupported = true;
    }else{
        //图片格式预览
        if(type.toLowerCase() == 'gif'||type.toLowerCase() == 'jpg'||type.toLowerCase() == 'png'||type.toLowerCase() == 'bmp'){
//            $window.open(basePath+"/pages/external/externalPage.jsp#/moraledu_showPicture?id="+decodeURIComponent($scope.attachmentUrl));
            window.location.href = basePath+"/pages/external/externalPage.jsp#/moraledu_showPicture?id="+decodeURIComponent(fastId);
        }else{
            //不符合的文件下载
            jumpPath = "/file/downloadStream.htm?fastDFSId="+fastId;
            isSupported = false;
        }
...
//TODO 如果不支持，直接下载
if(!previewFileOrDownload(fastDFSId)){
    alert("此文件格式不支持预览！点击下载。");
    window.location.href = basePath + "/file/download.htm?fastDFSId="+fastDFSId;
}


//  TODO 使用ajax同步发送请求
//TODO 此处注释的地方是原来的方法，是通过转化文件存放在临时文件中，通过拿临时文件来预览
//var initData = function(evt){
//    $.ajax({
//            url: basePath + "/file/previewFile.htm",
//            type: "post",
//            async: false,
//            dataType: "json",
//            data:{fastDFSId:fastDFSId},
...
//TODO 此处注释的地方是原来的方法，是通过转化文件存放在临时文件中，通过拿临时文件来预览
//var DEFAULT_URL = basePath+'/temp/previewFile.pdf';
var DEFAULT_URL = basePath + "/file/previewFile.htm?fastDFSId="+fastDFSId;
...
//TODO 此处注释的地方是原来的方法，是通过转化文件存放在临时文件中，通过拿临时文件来预览
//document.addEventListener('DOMContentLoaded', initData, true);
